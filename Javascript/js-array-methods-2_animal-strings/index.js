...
const catStartingFromIndexFiveExists = animalStrings.includes("cat") && animalStrings.index >5;
console.log ("Cat exists from index 5? " + catStartingFromIndexFiveExists);
...
const anyAnimalEndsWithLetterZ = animalStrings.every((animal) => animal.endsWith("z"));
console.log("Does any animal end with Z? " + anyAnimalEndsWithLetterZ);

const everyAnimalHasMoreThanTwoLetters = animalStrings.every((animal) => animal.length > 2);
console.log("Does every animal have more than two letters? " + everyAnimalHasMoreThanTwoLetters);
